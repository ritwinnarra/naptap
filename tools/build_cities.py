...
for line in open(os.path.join(D, "cities15000.txt"), encoding="utf-8"):
    f = line.rstrip("\n").split("\t")
    pop = int(f[14] or 0)
    if pop < MIN_POP or f[7] not in KEEP: continue
    rows.append({"name": f[1], "lat": float(f[4]), "lon": float(f[5]), "cc": f[8],
...
for name, country, lat, lon, note in curated:
    ...
    if km(best, c) <= 40:
        ...
        unmatched += 1; print("no GeoNames match within 40 km:", name, country)
        rows.append({"name": name, "lat": lat, "lon": lon, "cc": "", "country": country, "region": "", "pop": 0, "note": note, "curated": True})
...
for r in sorted(rows, key=lambda r: -r["pop"]):
    country = r["country"]
    if r["name"] in dupes and r["region"] and r["region"] != r["name"] and not r.get("curated"):
        country = f"{r['region']}, {country}"
    out.append([r["name"], country, round(r["lat"], 3), round(r["lon"], 3), r.get("note") or popnote(r["pop"])])
...
with open(os.path.join(ROOT, "cities.js"), "w", encoding="utf-8") as f:
    ...
    f.write("// [name, country, lat, lon, note]\nconst CITIES = [\n")
